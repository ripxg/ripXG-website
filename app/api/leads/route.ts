...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { name, email, message, useCases } = body as {
      name?: string;
      email?: string;
      message?: string;
      useCases?: string[];
    ...
    const useCasesHtml =
      useCases && useCases.length > 0
        ? useCases.map((uc) => `<li style="margin-bottom: 4px;">${uc}</li>`).join("")
        : "<em>none selected</em>";
    ...
    const html = `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
        ...
        <table style="width: 100%; border-collapse: collapse;">
          <tr>
            <td style="padding: 10px 0; border-bottom: 1px solid #e9d5ff; font-weight: bold; color: #4f1ca0; width: 140px;">Name</td>
          ...
          <tr>
            <td style="padding: 10px 0; border-bottom: 1px solid #e9d5ff; font-weight: bold; color: #4f1ca0; vertical-align: top;">Interested in</td>
            <td style="padding: 10px 0; border-bottom: 1px solid #e9d5ff; color: #374151;">
              <ul style="margin: 0; padding-left: 18px;">${useCasesHtml}</ul>
            </td>
          </tr>
          ${message ? `
          <tr>
            <td style="padding: 10px 0; font-weight: bold; color: #4f1ca0; vertical-align: top;">Message</td>
            <td style="padding: 10px 0; color: #374151; white-space: pre-wrap;">${message}</td>
          </tr>
          ` : ""}
        </table>
