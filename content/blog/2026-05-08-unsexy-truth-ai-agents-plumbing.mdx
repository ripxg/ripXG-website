...
1. OpenClaw catches the error. Not just "something broke" but it classifies the error type. Is it retryable? Is it a rate limit? Is it an auth failure?
2. If it's retryable, it backs off and tries again. Not immediately. Exponential backoff with jitter. Random delay between retries so multiple agents hitting the same provider don't thundering-herd back in.
...
4. If the provider itself is down, it walks the fallback chain. GLM-4.7, then Gemini Flash Lite, then a local Ollama model running on the VPS as last resort.
...
That's why I built [Orion AI](/orion-ai). You get the same infrastructure I've described in this post, the failover chains, the retry policies, the multi-model routing, the monitoring, the session management, without touching any of it.
